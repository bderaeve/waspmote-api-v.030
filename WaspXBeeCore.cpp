	...
	if( startSequence && !firm_info.already_init )
	{
		...
		if( !error_sd )
		{
			...
			if( !error_sd )
			{
				...
				if(file1.write(START_SECTOR,strlen(START_SECTOR))!=strlen(START_SECTOR))
				{
					error_sd=true;				
				}
				// Write program ID into firmware file				
				if(file1.write(firm_info.ID,strlen(firm_info.ID))!=strlen(firm_info.ID))				
				{
					error_sd=true;
				}
				
				// Write asterisks into firmware file				
				if(file1.write(asteriscos,strlen(asteriscos))!=strlen(asteriscos))				
				{
					error_sd=true;
				}
			}
...
void WaspXBeeCore::new_firmware_end()
{
	...
	if( send_ok )
	{
		...
		if(file2.write(firm_info.DATE,strlen(firm_info.DATE))!=strlen(firm_info.DATE))
		{
			error_sd=true;
		}	
...
void WaspXBeeCore::checkNewProgram(){
	...
	// check EEPROM[1] for new program running
	...
	//SD set up
	delay(100);
	// Open GND line to the SD card 
	pinMode(MEM_PW, OUTPUT);
	digitalWrite(MEM_PW, LOW);		
	delay(100);
	
	// Close SPI bus 
	pinMode(SD_SS,INPUT); // SS-->Input
	pinMode(SD_SCK,INPUT);// SCK-->Input
	pinMode(SD_MISO,INPUT);// MISO-->Input
	pinMode(SD_MOSI,INPUT);// MOSI-->Input
	delay(100);	
	// Close GND line to the SD card
	pinMode(MEM_PW, OUTPUT);
	digitalWrite(MEM_PW, HIGH);
	delay(100);
				...
				while( millis()-previous<60000 && !end_file)
				{
					...
					if( !end_file )
					{
						...
						if(!match_id)
						{
							...
							if(file1.write(buf_sd_aux,strlen(buf_sd_aux))!=strlen(buf_sd_aux))
							{
								error=true;
							}
							else match_id = true;
				...
				if(file1.open(&root, file_aux, O_READ))
				{
					...
					while( millis()-previous<60000 && !end_file)
					{
						...
						if( !end_file )
						{
							...
							// Write string to file
							if(file2.write(buf_sd_aux,strlen(buf_sd_aux))!=strlen(buf_sd_aux))
							{
								error=true;
							}
						}
